...
extension UIImageView {
    ...
    final func setImage(_ url: String?,
                        _ type: ImageStrategy = .bookOriginal,
                        ...
                        forOption options: KingfisherOptionsInfo = [.cacheMemoryOnly])
    {
        ...
        if let memoryCache = ImageCacheCenter.shared.image(forKey: dealURL.sha1()) {
            image = memoryCache
            return
        }
...
//                        PrintLog("当前进度 \(current), 总进度 \(totle)")
                        ...
                        if let _image = image, let _url = url {
                            KingfisherManager.shared.cache.store(_image, forKey: _url.absoluteString.sha1(), toDisk: false)
                        }
                    }
    }
