...
public extension Response {
    ...
    internal func mapResponse() throws -> ResponseModel {
        ...
        if serverModel.error == RequestCode.success.rawValue {
            return serverModel
        }else {
    ...
    internal func map<T: HandyJSON>(model type: T.Type) throws -> T {
        ...
        if serverModel.error == RequestCode.success.rawValue, let model = serverModel.Data {
            return model
        }else {
            throw MapperError.server(message: serverModel.message)
        }
    ...
    internal func map<T: HandyJSON>(model type: T.Type) throws -> [T] {
        ...
        if serverModel.error == RequestCode.success.rawValue, let models = serverModel.Data {
            return models
        }else {
            throw MapperError.server(message: serverModel.message)
        }
