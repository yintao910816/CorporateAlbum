...
extension ObservableType {
    ...
    static func selectedDB<T: DBOperation>(type: T.Type, tbName: String) ->Observable<[T]> {
        return Observable<[T]>.create { obser -> Disposable in
            DBQueue.share.selectQueue(type.selectedFilier()!, tbName, type) { table in
                obser.onNext(type.mapModel(query: table) as! [T])
                obser.onCompleted()
            }
