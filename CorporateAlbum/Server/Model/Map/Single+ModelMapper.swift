...
//    }
    ...
    /**
     * 匹配 ResponseStatusModel 模型
     */
    func mapResponseStatus() -> Single<ResponseStatusModel> {
        return observeOn(ConcurrentDispatchQueueScheduler(qos: .background))
            .flatMap { response -> Single<ResponseStatusModel> in
                return Single.just(try response._mapResponseStatus())
            }
            .observeOn(MainScheduler.instance)
    }
    ...
    func map<T: HandyJSON>(model type: T.Type) -> Single<T> {
        return observeOn(ConcurrentDispatchQueueScheduler(qos: .background))
            .flatMap { response -> Single<T> in
                return Single.just(try response._map(model: type))
            }
            .observeOn(MainScheduler.instance)
    }
    ...
    func map<T: HandyJSON>(models type: T.Type) -> Single<[T]> {
        return observeOn(ConcurrentDispatchQueueScheduler(qos: .background))
            .flatMap { response -> Single<[T]> in
                return Single.just(try response._map(models: type))
            }
            .observeOn(MainScheduler.instance)
    }
